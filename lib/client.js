window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 已清理的项（不想做了）：localStorage 按 sessionId 持久——2026-09-26 新增
    const loadDismissed = (sessionId) => {
      try {
        const raw = localStorage.getItem("dsh-todo-guard:dismissed:" + sessionId);
        const arr = raw ? JSON.parse(raw) : [];
        return new Set(Array.isArray(arr) ? arr : []);
      } catch (e) { return new Set(); }
    };
    const saveDismissed = (sessionId, set) => {
      try { localStorage.setItem("dsh-todo-guard:dismissed:" + sessionId, JSON.stringify([...set])); } catch (e) { /* 隐私模式忽略 */ }
    };
    // 每项首次出现的时间（标"遗留"用）
    const loadSeen = (sessionId) => {
      try {
        const raw = localStorage.getItem("dsh-todo-guard:seen:" + sessionId);
        const obj = raw ? JSON.parse(raw) : {};
        return obj !== null && typeof obj === "object" ? obj : {};
      } catch (e) { return {}; }
    };
    const saveSeen = (sessionId, obj) => {
      try { localStorage.setItem("dsh-todo-guard:seen:" + sessionId, JSON.stringify(obj)); } catch (e) { /* ignore */ }
    };
    ...
    function ReviewPanelView(props) {
      ...
      // 2026-09-26（用户："没啥用而且不太同步"）：
      // dismissed = 你不想做了的项——一键清掉，面板才能归零（此前只有全部完成才会消失）；
      // seen = 每项第一次出现的时间——用来把"上轮/更早留下的"标成遗留，跟官方面板对不上时一眼看懂。
      const [dismissed, setDismissed] = react.useState(() => loadDismissed(sessionId));
      const [seen, setSeen] = react.useState(() => loadSeen(sessionId));
      const [showDone, setShowDone] = react.useState(false);
      // 本次打开面板的时刻（标"遗留"用）——必须在提前 return 之前声明，否则违反 hooks 规则
      const openedAtRef = react.useRef(Date.now());
      react.useEffect(() => {
        const next = Object.assign({}, seen);
        let changed = false;
        for (const t of todos) {
          if (next[t.content] === undefined) { next[t.content] = Date.now(); changed = true; }
        }
        if (changed) { setSeen(next); saveSeen(sessionId, next); }
      }, [todosKey]);
      react.useEffect(() => {
      ...
      // 你清掉的项不再参与统计（"清理遗留"按钮写进 dismissed）
      const active = todos.filter((t) => !dismissed.has(t.content));
      const openItems = active.filter((t) => t.status !== "completed");
      const doneItems = active.filter((t) => t.status === "completed");
      const total = active.length;
      const done = doneItems.length;
      const incomplete = openItems.length;
      // 遗留＝不是这次打开面板之后才出现的（跨轮/跨会话留下来的）
      const openedAt = openedAtRef.current;
      const legacyCount = openItems.filter((t) => (seen[t.content] || openedAt) < openedAt - 1000).length;
      ...
      // 没有待办、也没有待确认 → 面板自动消失（不占你的输入框上方）
      if (incomplete === 0 && problemCount === 0) return null;
      ...
      // 一键清理：把当前所有未完成项标为"不想做了"（可在设置里恢复——重开面板即重新统计）
      const dismissOpen = () => {
        const next = new Set(dismissed);
        for (const t of openItems) next.add(t.content);
        setDismissed(next);
      ...
      const visibleProblems = problemCount > 0
        ? react.createElement("span", { className: "tg-badge tg-badgeWarn", title: "有完成项没交证据或证据路径不存在" },
          "⚠ " + problemCount + " 项待确认")
        : null;
      const legacyBadge = legacyCount > 0
        ? react.createElement("span", { className: "tg-badge tg-badgeMuted", title: "这些不是本轮新写的待办，是之前留下的（官方面板每轮会清空，所以两边看着不一样）" },
          "含遗留 " + legacyCount)
        : null;
      ...
      const renderItem = (item) => {
        ...
        const legacy = item.status !== "completed" && (seen[item.content] || openedAt) < openedAt - 1000;
        return react.createElement("li", { className: "tg-item", key: item.content },
          ...
          legacy ? react.createElement("span", { className: "tg-badge tg-badgeMuted", title: "之前留下的待办" }, "遗留") : null,
          evidenceText,
          badge);
      };
      ...
      return react.createElement("div", { className: "tg-root" },
        react.createElement("div", { className: "tg-headRow" },
          ...
          (incomplete > 0
            ? react.createElement("button", {
                type: "button", className: "tg-confirmBtn",
                title: "这些待办我不打算做了——从复查区清掉（不影响官方面板）",
                onClick: dismissOpen,
              }, "清理遗留")
            : null)),
        // 列表（折叠时不渲染）：先待办，已完成默认收起
        (!collapsed && react.createElement("ul", { className: "tg-list" },
          openItems.map(renderItem),
          dismissed.size > 0
            ? react.createElement("li", { className: "tg-item", key: "__dismissed-restore__" },
                react.createElement("span", { className: "tg-content", style: { cursor: "pointer", color: "var(--dsw-alias-label-tertiary)" },
                  onClick: () => { const empty = new Set(); setDismissed(empty); saveDismissed(sessionId, empty); } },
                  "已清理 " + dismissed.size + " 项（点此恢复）"))
            : null,
          doneItems.length > 0
            ? react.createElement("li", { className: "tg-item", key: "__done-toggle__" },
                react.createElement("span", { className: "tg-content", style: { cursor: "pointer", color: "var(--dsw-alias-label-tertiary)" }, onClick: () => setShowDone(!showDone) },
    ...
    function apply(ctx) {
      ...
      // 影子实测（2026-09-24）：key = `dsh-todo-guard#todo-guard` 命中的是「包详情 → 组件行」
      // 里那个 "Configure Todo Guard" 按钮打开的行详情页；卡片渲染在它的配置区（data-plugin-config）。
      const ROW_CONFIG_KEY = "dsh-todo-guard#todo-guard";
      slots.inject("plugins.row.config", () => slots.register(
        { name: "plugins.row.config", key: ROW_CONFIG_KEY, order: 30, label: "todo 严格模式" },
        renderItem,
      ));
      ...
      slots.inject("plugins.bundle.config", () => slots.register(
        { name: "plugins.bundle.config", key: "dsh-todo-guard", order: 30, label: "todo 严格模式" },
        renderItem,
      ));
