window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function hasProof(content) {
      // 与 host parseProofs 一致：要求闭合括号且有内容（两端统一，技术债 P2）；
      // 畸形标记（如无右括号的 （证据：xxx）→ false → 黄标兜底提醒
      return /[(（]\s*证据\s*[:：]\s*[^)）]+[)）]/.test(content);
    }
    ...
    function TodoPanelView(props) {
      ...
      const unverified = todos.filter(
        (t) => t.status === "completed" && verified && !hasProof(t.content)
      ).length;
      if (done === total) {
        ...
        return react.createElement("div", { className: "tg-root" },
          react.createElement("div", { className: "tg-body" },
            ...
            react.createElement("div", { style: { display: "flex", alignItems: "center", gap: "10px", fontSize: "13px", color: "var(--dsw-alias-state-warn-primary)" } },
              "其中 " + unverified + " 项未验证（缺（证据：路径）标记），确认收尾？"),
          ...
          !collapsed && react.createElement("ul", { className: "tg-list" },
            todos.map((item) => {
              ...
              const badge = verified && item.status === "completed" && !hasProof(item.content)
                ? react.createElement("span", { className: "tg-badge tg-badgeWarn", title: "完成项缺（证据：路径）标记，请确认任务确实完成" }, "未验证")
                : null;
