window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * 同 id 替换），数据走官方 useProjection('todo-guard/todos') 投影——本插件
     * host 侧注册的「最后有效 todo」镜像：官方 todos 单元在 turn/start 时清空
     * 为 null（面板/恢复会话随之空白），本投影不清空，保证跨 turn 延续显示、
     * 断会话/恢复会话后仍带出上次列表。展示三态：
    ...
    function hasTrace(content) {
      // v2 证据分级：格式证据（证据：路径）或内容里的文件路径锚点都算"有痕"；
      // 与 host extractAnchors 保守规则一致——只有真无痕迹才黄标"未验证"
      if (/[(（]\s*证据\s*[:：]\s*[^)）]+[)）]/.test(content)) return true;
      // 与 host extractAnchors 同步：先剥 URL（避免 :// 残留误判），再剥证据块；
      // 切分不用英文冒号（避免 Windows 盘符被切开）
    ...
    function TodoPanelView(props) {
      ...
      const unverified = todos.filter(
        (t) => t.status === "completed" && verified && !hasTrace(t.content)
      ).length;
      if (done === total) {
        // 全部完成：无未验证项 → 面板直接消失；有未验证项 → 收尾条确认后才消失
        if (unverified === 0 || finalized) return null;
        return react.createElement("div", { className: "tg-root" },
          react.createElement("div", { className: "tg-body" },
            ...
            react.createElement("div", { style: { display: "flex", alignItems: "center", gap: "10px", fontSize: "13px", color: "var(--dsw-alias-state-warn-primary)" } },
              "其中 " + unverified + " 项未验证（无任何可核验痕迹），确认收尾？"),
            react.createElement("button", {
              type: "button",
              onClick: () => setFinalized(true),
              style: { alignSelf: "flex-start", border: "1px solid var(--dsw-alias-border-l1)", background: "var(--dsw-specific-tip)", color: "var(--dsw-alias-label-primary)", borderRadius: "8px", padding: "4px 10px", fontSize: "12px", cursor: "pointer" },
            }, "确认收尾，收起面板")));
      }
      return react.createElement("div", { className: "tg-root" },
        react.createElement("div", { className: "tg-body" },
          ...
          !collapsed && react.createElement("ul", { className: "tg-list" },
            todos.map((item) => {
              ...
              const badge = verified && item.status === "completed" && !hasTrace(item.content)
                ? react.createElement("span", { className: "tg-badge tg-badgeWarn", title: "完成项无任何可核验痕迹（如具体文件路径），请确认任务确实完成" }, "未验证")
                : null;
    ...
    function SettingsCard() {
      ...
      return react.createElement("div", { style: { display: "flex", flexDirection: "column", gap: "8px", padding: "8px 0" } },
        ...
        react.createElement("h3", { style: { margin: 0, fontSize: 14, fontWeight: 600, color: "var(--dsw-alias-label-primary)" } }, "todo 校验"),
    ...
    function apply(ctx) {
      ...
      // 设置卡片：证据验证开关（侧边栏页 + 插件卡片，内容一致）
      slots.inject("settings.section", () => slots.register(
        { name: "settings.section", id: "dsh-todo-guard-settings", order: 40, label: "todo 校验" },
