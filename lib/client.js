window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // ── 插件区（0.1.7 官方插件管理页）──
      // 官方正路：把插件的详情页注册进 `plugins.item`，键 = profile 条目 id
      // （官方 ui-settings-shell / agent-loop 同款）。列表行给一行摘要，详情页渲染同一张设置卡。
      // 试用阶段两个候选 id 都注册（包名 / 条目 id）——不匹配的那个永远不会渲染，验证后删掉多余的。
      const itemViews = {
      ...
      for (const id of ["dsh-todo-guard", "todo-guard"]) {
        slots.inject("plugins.item", () => slots.register(
          { name: "plugins.item", id, order: 30, label: "todo 严格模式" },
          renderItem,
        ));
      }
