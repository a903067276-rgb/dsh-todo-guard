...
export function apply(ctx, config) {
  ...
  // 催办降噪（2026-09-26 用户说"不好用"后自评）：同一条"没交证据"不重复唠叨。
  // 按会话记住上次催过的条数，只有又多出完成项（数量变大）才再提一次——
  // 此前每调一次 todo_write 就催一遍，实测一个会话被催三次，纯噪音。
  const lastEvidenceNag = new Map() // sessionId -> { count, at }
  const EVIDENCE_NAG_INTERVAL_MS = 10 * 60 * 1000
  function shouldNagEvidence(sessionId, count) {
    const prev = lastEvidenceNag.get(sessionId)
    const t = Date.now()
    // 两种情况下才再催：没交的项变多了，或者距上次催已过 10 分钟（免得"每轮只勾一项"就永远不再提醒）
    if (prev !== undefined && count <= prev.count && t - prev.at < EVIDENCE_NAG_INTERVAL_MS) return false
    lastEvidenceNag.set(sessionId, { count, at: t })
    return true
  }
  ...
  ctx.on('tools/post-execute', async (exec, result, next) => {
    try {
      ...
      if (newly.length > 0 && agent && agent.session) {
        ...
        if (noEvidence > 0 && shouldNagEvidence(agent.session.id, noEvidence)) {
          msgs.push(`${noEvidence} 项待交证据：调 todo_evidence（content 照抄条目文字）`)
        }
        if (badPaths.length > 0) {
          msgs.push(`${badPaths.length} 处证据路径不存在（复查区已标"证据异常"）：${badPaths.slice(0, 3).join('、')}${badPaths.length > 3 ? '…' : ''}`)
        }
      }
      // 2026-09-26（用户拍板"不好用"后自评）：2~3 项一起勾是正常批次，不再唠叨；
      // 只有一次勾 ≥4 项（多半是"攒着一起交差"）才轻提醒一句。
      if (newly.length >= 4) {
