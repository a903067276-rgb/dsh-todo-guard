...
- **实用化改造（v2.5.0，用户反馈「没啥用而且不太同步」）**——
  - **能清掉**：头部多一个「清理遗留」按钮，不想做的待办一键清出复查区（官方面板不受影响），清完若没有别的事，面板自己消失；误清了还有「已清理 N 项（点此恢复）」。
  - **已完成默认收起**：列表先列待办，已完成的收成一行「已完成 N 项」，点开才看。
  - **标明遗留**：不是本轮新写的待办会带「遗留」标记，头部也写「含遗留 N」——官方面板每轮清空、复查区故意保留，两边看着不一样是设计使然，现在一眼能看懂。
  - **没待办也没待确认 → 面板不占位置**（此前只要有旧待办就一直挂着）。
  - 想让复查区彻底退化成「官方 todo 的续命版」（不要证据核验那套）：设置卡片里把「证据校验」关掉即可。
